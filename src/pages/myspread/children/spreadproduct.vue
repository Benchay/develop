      ...
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple" style="float: right;">
            ...
            <el-button style="padding: 8px 20px;" @click="refreshplan()">搜  索</el-button>
          </div>
        </el-col>
      </el-row>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        picktime: [new Date(2018, 2, 1, 0, 0), new Date(2018, 3, 11, 10, 10)],
        station: '',
        stationList: [
          {
            value: 'New York',
    ...
    methods: {
      ...
      refreshplan(){
        var _this=this
        var startTime=this.picktime[0]
        var endTime=this.picktime[1]
        callApiForMbs('plan/query_promote_plan', {pageNum:1,pageSize:10,startTime:startTime,endTime:endTime}, function (res) {
          if(res.data.success){
      ...
      refreshplancount(){
        ...
        var startTime=this.picktime[0]
        var endTime=this.picktime[1]
        callApiForMbs('plan/query_promote_plan_summary', {pageNum:1,pageSize:10,startTime:startTime,endTime:endTime}, function (res) {
          if(res.data.success){
            _this.tableData = res.data.content.records
          }else{
            alert(res.data.errmsg)
