      ...
      <el-col :span="24">
        <div class="title-info">
          ...
          <span class="finance-info">
            <span >当前余额:</span>
            <span class="show-finance-data">{{userAmount}}</span>
          </span>
        </div>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    onSubmit () {
      let me = this
      //
      callApiForMbs('/finance/recharge_amount', this.form, (res) => {
        if (res.status>=200 && res.status<300) {
          if (res.data.success) {
            me.$message({message: '支付成功，请等待审核', type: 'success'})
            return
          }
          me.$message.error('充值失败,失败原因：' + res.data.errmsg)
        }
        me.$message.error('充值失败,失败原因, 请联系客服解决')
      })
    }
  },
  created: function () {
    ...
    callApiForMbs('/finance/get_finance_account', {}, function(res) {
      if (res.status >= 200 && res.status < 300) {
        if (res.data.success) {
          me.userAmount = res.data.content.amount
        }
      }
    })
    callApiForMbs('/finance/query_receiver_config', {type: 1, payType: 1, status: 1, pageNum: 1, pageSize: 10}, function(res) {
      console.log(res)
      if (res.status >= 200 && res.status < 300) {
        if (res.data.success) {
